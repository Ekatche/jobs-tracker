...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      if (!Array.isArray(apiData)) throw new Error("Format de données invalide");
  ...
  // chargement initial
  useEffect(() => {
    fetchData().finally(() => { isInitialLoad.current = false; });
  }, [fetchData]);
  ...
  const currentApplications = activeTab === 'active' ? activeApplications : archivedApplications;
